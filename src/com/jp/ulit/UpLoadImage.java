...
public class UpLoadImage {
	 ...
	 * 人人网Photos.upload中的”程序调用“http://wiki.dev.renren.com/wiki/Photos.upload#.E7
	 * .A8.8B.E5.BA.8F.E8.B0.83.E7.94.A8 对其格式解释的非常清晰。 格式如下所示：
	...
	private void addImageContent(File[] files, DataOutputStream output) {
		for (File file : files) {
			...
			split.append("Content-Disposition: form-data; name= \"file\"; filename=\"" + file.getName()
					+ "\"" + lineEnd);
			split.append("Content-Type: image/" + FileUtil.getFileExtensionName(file) + lineEnd);
			split.append(lineEnd);
			try {
				byte[] fileByte = FileUtil.getByteArray(file);
				// 发送图片数据
	...
	/*
	 * 构建表单字段内容，格式请参考HTTP 协议格式（用FireBug可以抓取到相关数据）。 
	 * (以便上传表单相对应的参数值) 格式如下所示：
	 * --****************fD4fH3hK7aI6 Content-Disposition: form-data;
	...
	private void addFormField(Map<String, String> params, DataOutputStream output) {
		...
		Iterator iterator = keySet.iterator();
		 while(iterator.hasNext()) {
			...
			sb.append(lineEnd);
			sb.append(params.get(key) + lineEnd);
		}
		try {
